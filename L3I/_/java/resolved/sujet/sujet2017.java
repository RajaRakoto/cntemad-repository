...
public class sujet2017 {
	...
	public static void exo1(){
	  ...
	  public static void exo5 (){
		  ...
		  //declaration d'un tableau de type string 
		  String[] ville = {"Tana", "Diego", "Majunga", "Tamatave"};
//		  String[] ville = {"a", "f", "b", "z"};
		  String permut;
		  
		  //parcours du tableau + permutation de valeur 
		  for (int i=0; i<ville.length; i++){
			  for (int j=1; j<ville.length; j++){
				  if (ville[i].compareToIgnoreCase(ville[j]) > 0 && j>i){
					  permut = ville[j];
					  ville[j] = ville[i];
					  ville[i] = permut;
				  }
